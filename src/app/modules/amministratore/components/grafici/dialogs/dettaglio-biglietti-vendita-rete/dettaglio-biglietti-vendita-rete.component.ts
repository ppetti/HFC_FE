...
@Component({
  selector: 'app-dettaglio-biglietti-vendita-rete',
  templateUrl: './dettaglio-biglietti-vendita-rete.component.html',
  styleUrls: ['./dettaglio-biglietti-vendita-rete.component.scss']
})
export class DettaglioBigliettiVenditaReteComponent implements OnInit {
  bigliettiPerTipo;
  ...
  totaleFooter = null;
  totaleBiglietti = null;

  dataDa;
  dataA;
  formMacro: FormGroup;

  displayedColumns: string[] = ['titolo', 'numeroBiglietti', 'prezzoValidazioneTotale'];
  ...
  submit(){
    ...
    this.amministratoreService.getAllBigliettiVendutiValidatiReteDataFiltrati(wrapper).subscribe(
      responseTipologie => {
        ...
        listaProvvisoria.forEach(tipo => {
          ...
          listaAccorpata.forEach(el => {
            if(tipo.tipologiaBiglietto.titolo == el.titolo){
              if(tipo.prezzoValidazione != null || tipo.prezzoValidazione > 0){
                el.prezzoValidazioneTotale = el.prezzoValidazioneTotale + tipo.prezzoValidazione;

              } else {
                if(tipo.bigliettoFull){
                  el.prezzoValidazioneTotale = el.prezzoValidazioneTotale + tipo.tipologiaBiglietto.prezzo_full
                } else {
                  el.prezzoValidazioneTotale = el.prezzoValidazioneTotale + tipo.tipologiaBiglietto.prezzo_child
                  ;

                }
              }

              el.totBiglietti = el.totBiglietti + 1
              aggiunto = true;
          ...
          if(!aggiunto){
            ...
            if(tipo.prezzoValidazione != null || tipo.prezzoValidazione > 0){
              oggetto = {
                ...
                totBiglietti: 1,
                prezzoValidazioneTotale: tipo.prezzoValidazione,
              }
              ...
              if(tipo.bigliettoFull){
                oggetto = {
                  titolo: tipo.tipologiaBiglietto.titolo,
                  totBiglietti: 1,
                  prezzoValidazioneTotale: tipo.tipologiaBiglietto.prezzo_full,
                }
              } else {
                oggetto = {
                  titolo: tipo.tipologiaBiglietto.titolo,
                  totBiglietti: 1,
                  prezzoValidazioneTotale: tipo.tipologiaBiglietto.prezzo_child,
                }
              }
            }
        ...
        this.totaleFooter = 0;
        this.totaleBiglietti = 0;

        listaAccorpata.forEach(el => {
          this.totaleFooter += el.prezzoValidazioneTotale;
          this.totaleBiglietti += el.totBiglietti;
        });
  ...
  downloadExcel(){
    ...
    tipologieVendutiFiltratiCopy.push( {titolo: "TOTALI", tot: this.totaleFooter})
